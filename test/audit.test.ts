...
describe('auditLog', () => {
  it('logs just the tool name when there are no parameters', () => {
    expect(capture('list_buckets')).toBe('[storj-mcp] AUDIT: list_buckets');
    expect(capture('list_buckets', {})).toBe('[storj-mcp] AUDIT: list_buckets');
  });
  ...
  it('formats each value kind', () => {
    expect(capture('t', { bucket: 'b', n: 0, f: false, y: true, neg: -1.5, arr: [1, 2, 3], empty: [], obj: { a: 1 }, nul: null, d: new Date(0) }))
      .toBe('[storj-mcp] AUDIT: t bucket="b" n=0 f=false y=true neg=-1.5 arr=[3 items] empty=[0 items] obj={…} nul={…} d={…}');
  });

  it('previews strings longer than 100 chars but logs exactly 100 in full', () => {
    expect(capture('t', { s: 'x'.repeat(101) })).toBe(`[storj-mcp] AUDIT: t s="${'x'.repeat(50)}…" (101 chars)`);
    expect(capture('t', { s: 'x'.repeat(100) })).toBe(`[storj-mcp] AUDIT: t s="${'x'.repeat(100)}"`);
  });

  it('skips undefined values while keeping insertion order', () => {
    expect(capture('t', { u: undefined, v: 'x', w: undefined, z: 1 })).toBe('[storj-mcp] AUDIT: t v="x" z=1');
  });
  ...
  it.each(['content', 'access_grant', 'passphrase', 'api_key'])('redacts %s whatever its value', (field) => {
    for (const value of ['SUPERSECRET', 'x'.repeat(200), 12345, ['SUPERSECRET']]) {
      ...
      expect(line).toBe(`[storj-mcp] AUDIT: t ${field}=[REDACTED]`);
      expect(line).not.toContain('SUPERSECRET');
    }
  });

  it('does not log an undefined value even for a redacted field', () => {
    expect(capture('t', { content: undefined })).toBe('[storj-mcp] AUDIT: t');
  });
});
